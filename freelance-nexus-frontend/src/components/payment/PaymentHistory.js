...
const PaymentHistory = () => {
  ...
  const handleVerify = async (txnId) => {
    try {
      const res = await paymentService.verifyPayment(txnId);
      alert(res.message || "Payment verified!");
  ...
  const handleReceipt = (paymentId) => {
    navigate(`/payment/${paymentId}/receipt`);
  };
...
<th>Date</th>
<th>Project</th>
<th>{user?.roles?.includes('CLIENT') ? 'Freelancer' : 'Client'}</th>
<th>Amount</th>
<th>Method</th>
<th>Status</th>
<th>Actions</th>
...
<button
                            ...
                            onClick={() => handleVerify(payment.transactionId)}
>
                            Verify
</button>
                        )}
...
<h3 className="text-success">₹{payments.reduce((sum, p) => sum + p.amount, 0)}</h3>
</div>
</div>
</div>
